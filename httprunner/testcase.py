...
'''
2019-07-29 修改了eval_content_with_bindings方法，为了实现请求入参key中包含特殊字符'$'与httpRunner中的变量定义冲突问题，屏蔽这部分变量提取
'''
    ...
    def load_testsets_by_path(path):
        ...
        if os.path.isdir(path):
            ...
            try:
                ...
                '''修改源码：增加key是否存在判断'''
                if testset["testcases"] or ('api' in testset.keys() and testset["api"]):
                    testcases_list = [testset]
                else:
                    testcases_list = []
                    raise Exception('用例执行错误，请检查', '用例名称：'+testset.get('config', {}).get('name'))
...
class TestcaseParser(object):
    ...
    def eval_content_with_bindings(self, content, mask_key=False, extract_type=None):
        ...
        if isinstance(content, (list, tuple)):
            return [
                self.eval_content_with_bindings(item, mask_key)
                for item in content
            ]
        ...
        if isinstance(content, dict):
            evaluated_data = {}
            for key, value in content.items():
                # 进行调整，请求信息包含'$'符号于httpRunner变量冲突，临时屏蔽请求参数中key的参数提取，如：{"$name":"12412341234"}
                if key == 'json' and 'headers' in content.keys():
                    eval_key = self.eval_content_with_bindings(key, True)
                    eval_value = self.eval_content_with_bindings(value, True)
                else:
                    eval_key = self.eval_content_with_bindings(key, mask_key, 'key')
                    eval_value = self.eval_content_with_bindings(value, mask_key, 'value')
        ...
        if isinstance(content, basestring):
            ...
            # 屏蔽了就不提取变量
            if mask_key is False or extract_type != 'key':
