...
class HttpSession(requests.Session):
    ...
    def request(self, method, url, name=None, **kwargs):
        ...
        datas = json.dumps(kwargs.get('json', {}))
        token = kwargs.get('headers', {}).get('token', '')
        # 签名生成规则：md5(md5(token).toUpperCase() + timestamp + JSON.stringify(body || {})).toUpperCase()
        params = self.md5(token) + str(now_timestamp) + datas
        sign = self.md5(params)
        kwargs.get('headers', {})['timestamp'] = str(now_timestamp)
        kwargs.get('headers', {})['sign'] = sign
