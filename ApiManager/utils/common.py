...
from ApiManager.utils.operation import add_project_data, add_module_data, add_case_data, add_config_data, \
    add_register_data, add_logic_case_data
...
def load_cases(type=1, **kwargs):
    ...
    if module == '请选择':
        return ''
    case_info = TestCaseInfo.objects.filter(belong_module=module, type=type). \
...
def package_test_data(type=True, **kwargs):
    """
       用例信息逻辑处理以数据处理
...
def run_request_case(**kwargs):
    test = kwargs.pop('test')

    logging.info('调试用例: {kwargs}'.format(kwargs=kwargs))
    if test.get('name').get('case_name') is '':
        test['name']['case_name'] = str(uuid.uuid1())
    if test.get('name').get('module') == '请选择':
        test['name']['module'] = '缺省模块'
    if test.get('name').get('project') == '请选择':
        test['name']['project'] = '缺省项目'
    if test.get('name').get('project') == '':
        test['name']['project'] = '缺省项目'
    if test.get('name').get('module') == '':
        test['name']['module'] = '缺省模块'
...
def set_filter_session(request):
    ...
    if 'module_name' in request.POST.keys():
        request.session['module_name'] = request.POST.get('module_name')
    if 'executor' in request.POST.keys():
        request.session['executor'] = request.POST.get('executor', '')
    ...
    module_name = request.session.get('module_name')
    if not module_name:
        module_name = ''
        ...
        for test_case in content:
            ...
            if 'test' in test_case.keys():  # 忽略config
                ...
                if '/' in test_case['test']['name']:
                    test_case['test']['name'] = str(test_case['test']['name']).replace('/', '-') + '-' + str(version)
...
# 处理逻辑用例数据
def logic_test_case(type=True, **kwargs):
    data = package_test_data(type, **kwargs)
    if isinstance(data, str):
        return data
    return add_logic_case_data(type, **data)
