...
def customer_pager(base_url, current_page, total_page,total):
    """
    返回可分页的html
    ...
    total = "<li style='color:#0e90d2'>总数</li>：<li  style='color:#0e90d2'>%d</li>" % total
    pager_list.append(total)
...
def get_pager_info(Model, filter_query, url, id, per_items=12):
    ...
    if filter_query:
        ...
        case_id = filter_query.get('case_id')
        service_name = filter_query.get('service_name')
        module_name = filter_query.get('module_name')
    ...
    if url == '/api/project_list/':
        ...
        if filter_query.get('project_name'):
            obj = obj.filter(project_name__contains=filter_query.get('project_name'))

    elif url == '/api/module_list/':
        if belong_project != 'All':
            obj = obj.filter(belong_project__project_name__contains=belong_project)
        if module_name:
            obj = obj.filter(module_name__contains=module_name)
        if user:
            obj == obj.filter(test_user__contains=user)

    elif url == '/api/service_config_list/':
        if filter_query.get('service_name'):
            obj = obj.filter(service_name__contains=filter_query.get('service_name'))
        if filter_query.get('service_version'):
            obj = obj.filter(service_version__contains=filter_query.get('service_version'))

    elif url == '/api/report_list/':
        obj = obj.filter(report_name__contains=filter_query.get('report_name'))
        if filter_query.get('type'):
            obj = obj.filter(type=filter_query.get('type'))
        if filter_query.get('status'):
            obj = obj.filter(status=filter_query.get('status'))
        if filter_query.get('executor'):
            obj = obj.filter(executor=filter_query.get('executor'))
        ...
        if case_id is not None and case_id != '':
            obj = obj.filter(id=case_id)
        if service_name is not None and service_name != '':
            obj = obj.filter(service_name=service_name)
